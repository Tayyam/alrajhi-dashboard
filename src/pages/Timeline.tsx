...
function getNodeFill(status: NodeStatus, i: number, total: number): { fill: string; stroke: string } {
...
function findCurrentIdx(nodes: NodeRow[]): number {
  const now = Date.now();
  const firstFutureIdx = nodes.findIndex((n) => new Date(n.date).getTime() > now);
  if (firstFutureIdx < 0) return nodes.length - 1;
  if (firstFutureIdx === 0) return 0;
  return firstFutureIdx;
}
      ...
      <div className="w-full max-w-[1990px] mx-auto lg:pr-15 lg:pl-25 pl-5 md:pl-10">
        <svg viewBox="0 0 1600 884" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" direction="ltr">
          ...
          {points.length > 0 && (() => {
            ...
            return (
              <g className="header-fade">
                ...
                <image
                  href={assetUrl(LOGO_KEY)} x={hx - 130} y={hy - 135} width="260" height="120"
          ...
          {points.map(([cx, cy], i) => {
            ...
            const { fill, stroke } = getNodeFill(status, i, nodes.length);
            return (
